...
sed -i 's|^#!/bin/sh$|#!/usr/bin/env bash|' apr-config.in
sed -i 's|@prefix@|$(cd $(dirname $(readlink -f ${BASH_SOURCE[0]}))/%{prefix_bin_rel}; pwd)|' apr-config.in
sed -i 's|@exec_prefix@|$(cd $(dirname $(readlink -f ${BASH_SOURCE[0]}))/%{exec_prefix_bin_rel}; pwd)|' apr-config.in
sed -i 's|@libdir@|${prefix}/%{lib_prefix_rel}|' apr-config.in
sed -i 's|@bindir@|${prefix}/%{bin_prefix_rel}|' apr-config.in
sed -i 's|@datadir@|${prefix}/%{data_prefix_rel}|' apr-config.in
sed -i 's|@includedir@|${prefix}/%{include_prefix_rel}|' apr-config.in
sed -i 's|@installbuilddir@|${prefix}/%{localstate_prefix_rel}/apr|' apr-config.in

%build
%configure --with-installbuilddir=%{_localstatedir}/apr \
